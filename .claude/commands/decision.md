...
- `-b, --branch <name>` - jj bookmark (auto-detected by default)
- `--no-branch` - Skip bookmark auto-detection
- `--commit <hash|HEAD>` - Link to commit (use HEAD for current)

### ⚠️ CRITICAL: Link Commits to Actions/Outcomes

**After every jj commit, link it to the decision graph!**

```bash
jj commit -m "feat: add auth"
...
**Nodes are automatically tagged with the current jj bookmark.** This enables filtering by feature/PR.

### How It Works
- When you create a node, the current jj bookmark is stored in `metadata_json`
- Configure which bookmarks are "main" in `.deciduous/config.toml`:
  ```toml
  [branch]
  main_branches = ["main", "master"]  # Bookmarks not treated as "feature bookmarks"
  auto_detect = true                   # Auto-detect bookmark on node creation
  ```
- Nodes on feature bookmarks (anything not in `main_branches`) can be grouped/filtered

### CLI Filtering
```bash
# Show only nodes from specific bookmark
deciduous nodes --branch main
deciduous nodes --branch feature-auth
deciduous nodes -b my-feature

# Override auto-detection when creating nodes
deciduous add goal "Feature work" -b feature-x  # Force specific bookmark
deciduous add goal "Universal note" --no-branch  # No bookmark tag
```

### Web UI Bookmark Filter
The graph viewer shows a bookmark dropdown in the stats bar:
- "All bookmarks" shows everything
- Select a specific bookmark to filter all views (Chains, Timeline, Graph, DAG)

### When to Use Bookmark Grouping
- **Feature work**: Nodes created on `feature-auth` bookmark auto-grouped
- **PR context**: Filter to see only decisions for a specific PR
- **Cross-cutting concerns**: Use `--no-bookmark` for universal notes
- **Retrospectives**: Filter by bookmark to see decision history per feature
...
**Problem**: Multiple users work on the same codebase, each with a local `.deciduous/deciduous.db` (ignored in VCS). How to share decisions?

**Solution**: Dual-ID model. Each node has:
- `id` (integer): Local database primary key, different per machine
- `change_id` (UUID): Globally unique, stable across all databases

### Export Workflow
```bash
# Export nodes from your bookmark as a patch file
