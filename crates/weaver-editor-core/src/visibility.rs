...
impl VisibilityState {
    ...
    /// Get the number of visible spans.
    pub fn visible_count(&self) -> usize {
        self.visible_span_ids.len()
    }

    /// Check if any spans are visible.
    pub fn has_visible(&self) -> bool {
        !self.visible_span_ids.is_empty()
    }
}
...
mod tests {
    ...
    fn make_para(start: usize, end: usize, syntax_spans: Vec<SyntaxSpanInfo>) -> ParagraphRender {
        ParagraphRender {
            id: format_smolstr!("test-{}-{}", start, end),
    ...
    fn test_selection_reveals_syntax() {
        ...
        let selection = Selection::new(3, 10);
